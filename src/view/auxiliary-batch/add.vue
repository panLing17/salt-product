<template>
    <transition name="fade">
        <div class="mask auxiliary-bath-add" v-if="maskShow">
...
<script>
  export default {
    ...
    methods: {
      ...
      getMaterialsInfo() {
        this.$http({
          url: this.$api + 'produce/resources/rs/excipient/getById',
          method: 'post',
          data: {
            reqParam: {
              pkId: this.materials
      ...
      right () {
        ...
        this.$http({
          url: this.$api + 'produce/production/pd/excipient/batch/save',
          method: 'post',
          data: this.params
        }).then(res => {
          if(res.data.retCode === 1) {
